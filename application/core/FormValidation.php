...
{
    //Singleton instance of FormValidation
     ...
     * @param array $validators The FormValidation validators
     *
     * @return mixed True(boolean) or the array of error messages
     */
    public static function is_valid(array $data, array $validators)
    {
        $FormValidation = self::get_instance();

        $FormValidation->validation_rules($validators);

        if ($FormValidation->run($data) === false) {
			
            return $FormValidation->get_readable_errors(true);
        } else {
            return true;
        }
    }
    ...
    public static function filter_input(array $data, array $filters)
    {
        ...
        return $FormValidation->filter($data, $filters);
    }
     ...
     * Usage:
     *
     * FormValidation::set_field_names(array(
     * 	"name" => "My Lovely Name",
     ...
     * @return string
     */
    public function get_readable_errors($convert_to_string = false, $field_class = 'FormValidation-field', $error_class = 'FormValidation-error-message')
    {
    ...
    */
     ...
     *
     * See: https://github.com/Wixel/FormValidation/issues/5
    ...
    protected function validate_date($field, $input, $param = null)
    {
        ...
        $cdate1 = date('d-m-Y', strtotime($input[$field]));
    ...
    protected function validate_min_age($field, $input, $param = null)
    {
        ...
        $cdate1 = new DateTime(date('d/m/Y', strtotime($input[$field])));
